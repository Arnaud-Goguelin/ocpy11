...
class TestFunctional:
    ...
    def test_login_book_places_logout(self):
        ...
        try:
            ...
            submit_button = driver.find_element("tag name", "button")
            submit_button.click()
            self._iron_temple["points"] = int(self._iron_temple["points"]) - self._places_booked
            self._future_competition["numberOfPlaces"] = int(self._future_competition["numberOfPlaces"]) - self._places_booked
            assert "Great-booking complete!" in driver.page_source
            assert f"Points available: {self._iron_temple["points"]}" in driver.page_source
            assert f"Number of Places: {self._future_competition["numberOfPlaces"]}" in driver.page_source
